...
window.startChat = (uid, name) => {
    // যদি এআই বটের সাথে চ্যাট শুরু হয়, তবে তা সরাসরি বটের নিজস্ব ফ্লোতে পাঠিয়ে দেওয়া হবে
    if (uid === 'smart_bot_ira') {
        if (typeof window.startBotChat === 'function') {
            window.startBotChat();
        } else {
            // যদি bot.js লোড না হয়ে থাকে, তবে তা ডাইনামিকালি লোড করে স্টার্ট করা হবে
            const script = document.getElementById('ai-bot-script');
            if (!script) {
        ...
        return; // ফায়ারবেস মেসেজ লোডিং স্কিপ করা হলো
    }
...
window.closeChat = () => {
    ...
    const wasBot = window.currentChatUser && window.currentChatUser.uid === 'smart_bot_ira';
    ...
    // এআই বটের জন্য ডেটাবেসে টাইপিং রাইট বন্ধ রাখা হলো যাতে পারমিশন এরর না আসে
    if(window.currentUser && !wasBot) {
        set(ref(window.db, `chats_typing/${window.currentUser.uid}`), false).catch(e => {});
    }
